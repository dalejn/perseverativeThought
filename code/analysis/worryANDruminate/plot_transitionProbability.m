...
for J = 0
    ...
    for K = 1:2
        ...
        caxis_bound = max(max(abs(nBackMinusRestTP.*~eye(numClusters))));
        h = colorbar; caxis([-caxis_bound caxis_bound]); h.Ticks = [-caxis_bound 0 caxis_bound]; h.TickLabels = [round(-caxis_bound,2,'significant') 0 round(caxis_bound,2,'significant')];
        COLOR_TICK_LABELS(true,true,numClusters);
        % title([thought_type, ', ', group_types{2}, '-', thought_type, ', ', group_types{1}], 'FontSize', 5);
        title([group_types{2}, '-', group_types{1}]);
        set(gca,'FontSize',8);
        set(gca,'TickLength',[0 0]);
        set(gca,'Fontname','arial');
        ...
        elseif K==2
            sgtitle("Transition probabilities for worry/rumination between clinical and control groups")
        end
...
for J = 0:1
    ...
    if J==0
        sgtitle("Transition probability between neutral and worry/rumination within control group")
    elseif J==1
        sgtitle("Transition probability between neutral and worry/rumination within clinical group")
    end
    ...
    for K = 1:2
        ...
        if J==0
            if K==1
                ...
                sgtitle("Transition probability between 1-back and worry/rumination thought within control group")
            end
        elseif J==1
            if K==1
                sgtitle("Transition probability between 1-back and neutral thought within clinical group")
            else
                sgtitle("Transition probability between 1-back and worry/rumination thought within clinical group")
            end
        end
