...
if nparc > 400
    network7labels=dlmread('data/schaefer200x17CommunityAffiliation_binned.txt'); network7labels = network7labels(1:nparc);
else
    network7labels=dlmread('data/schaefer200x17CommunityAffiliation_binned.txt'); network7labels = network7labels(1:nparc);
end
...
YeoNetNames = {'VIS+', 'SOM+', 'DAT+', 'VAT+', 'LIM+', 'FPN+', 'DMN+', 'SUB+','VIS-', 'SOM-', 'DAT-', 'VAT-', 'LIM-', 'FPN-', 'DMN-', 'SUB-'};
